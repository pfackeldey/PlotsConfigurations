...
for m in masses:
    for model in models:
        ...
        nuisances['QCDscale_qqH']['samples'].update(
            {'qqH_hww_' + m + '_' + model_name: HiggsXS.GetHiggsProdXSNP('YR4prel', '13TeV', 'vbfH', m, 'scale', 'bsm')})
...
for m in masses:
    ...
    weight1 = ((0.971893 + (mass * 8.75961e-05) + (mass * mass * -3.9762e-08) +
                (mass * mass * mass * 6.78212e-12)) * (mass > 449) + 1.003 * (mass < 449))
    weight = str(weight1)
    # print  weight
    for model in models:
        model_name = model.replace("cprime", "c").replace(
            ".", "").replace("BRnew", "brn")
        nuisances['QCDscale_qqbar_accept']['samples'].update(
            {'qqH_hww_' + m + '_' + model_name: weight})
        nuisances['QCDscale_qqbar_accept']['samples'].update(
            {'qqH_hww_SBI' + m + '_' + model_name: weight})
...
for m in masses:
    ...
    for model in models:
        model_name = model.replace("cprime", "c").replace(
            ".", "").replace("BRnew", "brn")
        nuisances['QCDscale_gg_accept']['samples'].update(
            {'ggH_hww_' + m + '_' + model_name: weightA})
        nuisances['QCDscale_gg_accept']['samples'].update(
            {'ggH_hww_SBI' + m + '_' + model_name: weightA})
...
for m in masses:
    ...
    for model in models:
        ...
        nuisances['QCDscale']['samples'].update({'ggH_hww_' + m + '_' + model_name: [
            "(" + unc0jet + "*(std_vector_jet_pt[0] < 30)+" + unc1jet + "*(std_vector_jet_pt[0] > 30 && std_vector_jet_pt[1] < 30)+" + unc2jet +
            "*((std_vector_jet_pt[1]> 30 ) && (mjj<500 || detajj<3.5))+" +
            unc3jet + "*(std_vector_jet_pt[1]> 30)*(mjj>500 && detajj>3.5))",
            "(1./(" + unc0jet + "*(std_vector_jet_pt[0] < 30)+" + unc1jet + "*(std_vector_jet_pt[0] > 30 && std_vector_jet_pt[1] < 30)+" + unc2jet +
            "*((std_vector_jet_pt[1]> 30 ) && (mjj<500 || detajj<3.5))+" +
            unc3jet + "*(std_vector_jet_pt[1]> 30)*(mjj>500 && detajj>3.5)))"
        ]
        })
...
for m in masses:
    for model in models:
        model_name = model.replace("cprime", "c").replace(
            ".", "").replace("BRnew", "brn")
        nuisances['pdf_gg']['samples'].update(
            {'ggH_hww_' + m + '_' + model_name: HiggsXS.GetHiggsProdXSNP('YR4prel', '13TeV', 'ggH', m, 'pdf', 'bsm')})
        nuisances['pdf_gg']['samples'].update(
            {'ggH_hww_SBI' + m + '_' + model_name: HiggsXS.GetHiggsProdXSNP('YR4prel', '13TeV', 'ggH', m, 'pdf', 'bsm')})
...
for m in masses:
    for model in models:
        model_name = model.replace("cprime", "c").replace(
            ".", "").replace("BRnew", "brn")
        nuisances['pdf_qqbar']['samples'].update(
            {'qqH_hww_' + m + '_' + model_name: HiggsXS.GetHiggsProdXSNP('YR4prel', '13TeV', 'vbfH', m, 'pdf', 'bsm')})
        nuisances['pdf_qqbar']['samples'].update(
            {'qqH_hww_SBI' + m + '_' + model_name: HiggsXS.GetHiggsProdXSNP('YR4prel', '13TeV', 'vbfH', m, 'pdf', 'bsm')})
# Add qq_SBI
...
for m in masses:
    for model in models:
        model_name = model.replace("cprime", "c").replace(
            ".", "").replace("BRnew", "brn")
        if m < 1500:
            nuisances['pdf_gg_accept']['samples'].update(
                {'ggH_hww_' + m + '_' + model_name: '1.007'})
            nuisances['pdf_gg_accept']['samples'].update(
                {'ggH_hww_SBI' + m + '_' + model_name: '1.010'})
        if m > 1499:
            nuisances['pdf_gg_accept']['samples'].update(
                {'ggH_hww_' + m + '_' + model_name: '1.012'})
            nuisances['pdf_gg_accept']['samples'].update(
                {'ggH_hww_SBI' + m + '_' + model_name: '1.035'})
...
for m in masses:
    for model in models:
        model_name = model.replace("cprime", "c").replace(
            ".", "").replace("BRnew", "brn")
        if m < 1000:
            nuisances['pdf_qqbar_accept']['samples'].update(
                {'qqH_hww_' + m + '_' + model_name: '1.005'})
            nuisances['pdf_qqbar_accept']['samples'].update(
                {'qqH_hww_SBI' + m + '_' + model_name: '1.005'})
        if m > 999:
            nuisances['pdf_qqbar_accept']['samples'].update(
                {'qqH_hww_' + m + '_' + model_name: '1.015'})
            nuisances['pdf_qqbar_accept']['samples'].update(
                {'qqH_hww_SBI' + m + '_' + model_name: '1.015'})
...
for m in masses:
    for model in models:
        model_name = model.replace("cprime", "c").replace(
            ".", "").replace("BRnew", "brn")
        nuisances['lumi']['samples'].update(
            {'ggH_hww_' + m + '_' + model_name: '1.025'})
        nuisances['lumi']['samples'].update(
            {'qqH_hww_' + m + '_' + model_name: '1.025'})
        nuisances['lumi']['samples'].update(
            {'ggH_hww_SBI' + m + '_' + model_name: '1.025'})
        nuisances['lumi']['samples'].update(
            {'qqH_hww_SBI' + m + '_' + model_name: '1.025'})
...
if Nlep == '2':
    ...
    fakeW_EleUp = '( fakeW_ele_' + eleWP + '_mu_' + muWP + '_' + Nlep + \
        'lElUp       / fakeW_ele_' + eleWP + '_mu_' + muWP + '_' + Nlep + 'l )'
    fakeW_EleDown = '( fakeW_ele_' + eleWP + '_mu_' + muWP + '_' + Nlep + \
        'lElDown     / fakeW_ele_' + eleWP + '_mu_' + muWP + '_' + Nlep + 'l )'
    fakeW_MuUp = '( fakeW_ele_' + eleWP + '_mu_' + muWP + '_' + Nlep + \
        'lMuUp       / fakeW_ele_' + eleWP + '_mu_' + muWP + '_' + Nlep + 'l )'
    fakeW_MuDown = '( fakeW_ele_' + eleWP + '_mu_' + muWP + '_' + Nlep + \
        'lMuDown     / fakeW_ele_' + eleWP + '_mu_' + muWP + '_' + Nlep + 'l )'
    fakeW_statEleUp = '( fakeW_ele_' + eleWP + '_mu_' + muWP + '_' + Nlep + \
        'lstatElUp   / fakeW_ele_' + eleWP + '_mu_' + muWP + '_' + Nlep + 'l )'
    fakeW_statEleDown = '( fakeW_ele_' + eleWP + '_mu_' + muWP + '_' + Nlep + \
        'lstatElDown / fakeW_ele_' + eleWP + '_mu_' + muWP + '_' + Nlep + 'l )'
    fakeW_statMuUp = '( fakeW_ele_' + eleWP + '_mu_' + muWP + '_' + Nlep + \
        'lstatMuUp   / fakeW_ele_' + eleWP + '_mu_' + muWP + '_' + Nlep + 'l )'
    fakeW_statMuDown = '( fakeW_ele_' + eleWP + '_mu_' + muWP + '_' + Nlep + \
        'lstatMuDown / fakeW_ele_' + eleWP + '_mu_' + muWP + '_' + Nlep + 'l )'
...
for m in masses:
    for model in models:
        model_name = model.replace("cprime", "c").replace(
            ".", "").replace("BRnew", "brn")
        nuisances['btagbc']['samples'].update({'ggH_hww_' + m + '_' + model_name: [
                                              '(' + bSF + '_bc_up)/(' + bSF + ')', '(' + bSF + '_bc_down)/(' + bSF + ')']})
        nuisances['btagbc']['samples'].update({'qqH_hww_' + m + '_' + model_name: [
                                              '(' + bSF + '_bc_up)/(' + bSF + ')', '(' + bSF + '_bc_down)/(' + bSF + ')']})
        nuisances['btagbc']['samples'].update({'ggH_hww_SBI' + m + '_' + model_name: [
                                              '(' + bSF + '_bc_up)/(' + bSF + ')', '(' + bSF + '_bc_down)/(' + bSF + ')']})
        nuisances['btagbc']['samples'].update({'qqH_hww_SBI' + m + '_' + model_name: [
                                              '(' + bSF + '_bc_up)/(' + bSF + ')', '(' + bSF + '_bc_down)/(' + bSF + ')']})
...
for m in masses:
    for model in models:
        model_name = model.replace("cprime", "c").replace(
            ".", "").replace("BRnew", "brn")
        nuisances['btagudsg']['samples'].update({'ggH_hww_' + m + '_' + model_name: [
                                                '(' + bSF + '_udsg_up)/(' + bSF + ')', '(' + bSF + '_udsg_down)/(' + bSF + ')']})
        nuisances['btagudsg']['samples'].update({'qqH_hww_' + m + '_' + model_name: [
                                                '(' + bSF + '_udsg_up)/(' + bSF + ')', '(' + bSF + '_udsg_down)/(' + bSF + ')']})
        nuisances['btagudsg']['samples'].update({'ggH_hww_SBI' + m + '_' + model_name: [
                                                '(' + bSF + '_udsg_up)/(' + bSF + ')', '(' + bSF + '_udsg_down)/(' + bSF + ')']})
        nuisances['btagudsg']['samples'].update({'qqH_hww_SBI' + m + '_' + model_name: [
                                                '(' + bSF + '_udsg_up)/(' + bSF + ')', '(' + bSF + '_udsg_down)/(' + bSF + ')']})
...
if Nlep == '2':
    ...
    trig_syst = ['(effTrigW3l_Up)/(effTrigW3l)',
                 '(effTrigW3l_Down)/(effTrigW3l)']
...
for m in masses:
    for model in models:
        model_name = model.replace("cprime", "c").replace(
            ".", "").replace("BRnew", "brn")
        nuisances['trigg']['samples'].update(
            {'ggH_hww_' + m + '_' + model_name: trig_syst})
        nuisances['trigg']['samples'].update(
            {'qqH_hww_' + m + '_' + model_name:  trig_syst})
        nuisances['trigg']['samples'].update(
            {'ggH_hww_SBI' + m + '_' + model_name: trig_syst})
        nuisances['trigg']['samples'].update(
            {'qqH_hww_SBI' + m + '_' + model_name: trig_syst})


# Electron Efficiency and energy scale ##########ok!

id_syst_ele = ['LepSF' + Nlep + 'l__ele_' + eleWP +
               '__Up', 'LepSF' + Nlep + 'l__ele_' + eleWP + '__Do']
...
for m in masses:
    for model in models:
        model_name = model.replace("cprime", "c").replace(
            ".", "").replace("BRnew", "brn")
        nuisances['eff_e']['samples'].update(
            {'ggH_hww_' + m + '_' + model_name: id_syst_ele})
        nuisances['eff_e']['samples'].update(
            {'qqH_hww_' + m + '_' + model_name: id_syst_ele})
        nuisances['eff_e']['samples'].update(
            {'ggH_hww_SBI' + m + '_' + model_name: id_syst_ele})
        nuisances['eff_e']['samples'].update(
            {'qqH_hww_SBI' + m + '_' + model_name: id_syst_ele})
...
for m in masses:
    for model in models:
        model_name = model.replace("cprime", "c").replace(
            ".", "").replace("BRnew", "brn")
        nuisances['electronpt']['samples'].update(
            {'ggH_hww_' + m + '_' + model_name: ['1', '1']})
        nuisances['electronpt']['samples'].update(
            {'qqH_hww_' + m + '_' + model_name: ['1', '1']})
        nuisances['electronpt']['samples'].update(
            {'ggH_hww_SBI' + m + '_' + model_name: ['1', '1']})
        nuisances['electronpt']['samples'].update(
            {'qqH_hww_SBI' + m + '_' + model_name: ['1', '1']})


elePtCor_Syst = ['electron_ptW_' + Nlep + 'l_Up / electron_ptW_' + Nlep +
                 'l', 'electron_ptW_' + Nlep + 'l_Down / electron_ptW_' + Nlep + 'l']
nuisances['elePtCor'] = {
    'name': 'hww_elePtCor',
    'kind': 'weight',
    'type': 'shape',
...
for m in masses:
    for model in models:
        model_name = model.replace("cprime", "c").replace(
            ".", "").replace("BRnew", "brn")
        nuisances['elePtCor']['samples'].update(
            {'ggH_hww_' + m + '_' + model_name: elePtCor_Syst})
        nuisances['elePtCor']['samples'].update(
            {'qqH_hww_' + m + '_' + model_name: elePtCor_Syst})
        nuisances['elePtCor']['samples'].update(
            {'ggH_hww_SBI' + m + '_' + model_name: elePtCor_Syst})
        nuisances['elePtCor']['samples'].update(
            {'qqH_hww_SBI' + m + '_' + model_name: elePtCor_Syst})

eleEtaCor_Syst = ['electron_etaW_' + Nlep + 'l_Up / electron_etaW_' + Nlep +
                  'l', 'electron_etaW_' + Nlep + 'l_Down / electron_etaW_' + Nlep + 'l']
...
for m in masses:
    for model in models:
        ...
        nuisances['eleEtaCor']['samples'].update(
            {'ggH_hww_' + m + '_' + model_name:  eleEtaCor_Syst})
        nuisances['eleEtaCor']['samples'].update(
            {'qqH_hww_' + m + '_' + model_name:  eleEtaCor_Syst})
        nuisances['eleEtaCor']['samples'].update(
            {'ggH_hww_SBI' + m + '_' + model_name:  eleEtaCor_Syst})
        nuisances['eleEtaCor']['samples'].update(
...
for m in masses:
    for model in models:
        model_name = model.replace("cprime", "c").replace(
            ".", "").replace("BRnew", "brn")
        nuisances['eff_m']['samples'].update(
            {'ggH_hww_' + m + '_' + model_name: id_syst_mu})
        nuisances['eff_m']['samples'].update(
            {'qqH_hww_' + m + '_' + model_name: id_syst_mu})
        nuisances['eff_m']['samples'].update(
            {'ggH_hww_SBI' + m + '_' + model_name: id_syst_mu})
        nuisances['eff_m']['samples'].update(
            {'qqH_hww_SBI' + m + '_' + model_name: id_syst_mu})
...
for m in masses:
    for model in models:
        model_name = model.replace("cprime", "c").replace(
            ".", "").replace("BRnew", "brn")
        nuisances['muonpt']['samples'].update(
            {'ggH_hww_' + m + '_' + model_name: ['1', '1']})
        nuisances['muonpt']['samples'].update(
            {'qqH_hww_' + m + '_' + model_name: ['1', '1']})
        nuisances['muonpt']['samples'].update(
