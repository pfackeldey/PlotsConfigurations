...
for m in masses:
  for model in models:
    ...
    print m
    #my_list = ['120', '125', '130', '135', '140', '145', '150', '155', '160', '165', '170', '175', '180', '190', '200', '210', '230', '250', '270']
    ...
    if m  in my_list:
     ...
     samples['ggH_hww_'+m+'_'+model_name] = { 'name': getSampleFiles(directory,'GluGluHToWWTo2L2Nu_M'+m) ,
                                          ...
                                          'weight' : XSWeight+'*'+SFweight+'*'+GenLepMatch+'*'+METFilter_MC+'*'+model+"*"+str(sf), #replace "1." with "model"
                                          'suppressNegativeNuisances' :['all'],
                                        }
...
for Run in DataRun :
  ...
  #directory = '/net/scratch_cms3b/fackeldey/artus/HWW/wjets_data/'
...
for Run in DataRun :
  ...
  #directory = '/net/scratch_cms3b/fackeldey/artus/HWW/data/'
  for DataSet in DataSets :
    FileTarget = getSampleFiles(directory,DataSet+'_'+Run[1],True)
    for iFile in FileTarget:
